...
import { Plus, Pencil, Search, Trash2, AlertTriangle, Upload, Trash, Check } from 'lucide-react'
...
export default function UrunlerPage() {
  ...
  const margin = (p: any) => p.purchasePrice > 0 ? (p.salePrice - p.purchasePrice) / p.salePrice * 100 : -1
  ...
  const filtered = products.filter(p =>
    ...
    // Önce code'a göre sayısal sırala (A001 < A002)
    if (a.code && b.code) {
      return a.code.localeCompare(b.code, 'tr', { numeric: true })
    }
    // Code olmayanları sona at
    if (a.code && !b.code) return -1
    if (!a.code && b.code) return 1
    // İkisinde de code yoksa ID'ye göre
                  ...
                  {filtered.map(p => editingId === p.id ? (
                    ...
                    <tr key={`edit-${p.id}`} className="border-b bg-yellow-50" onKeyDown={(e) => {
                      ...
                      <td className="px-3 py-4 w-20">
                        {p.imageUrl ? (
                          <img src={p.imageUrl} alt={p.name} className="h-20 w-20 object-contain rounded border bg-white" />
                        ) : (
                          <div className="h-20 w-20 bg-gray-100 rounded flex items-center justify-center text-base text-gray-400">Yok</div>
                        )}
                      </td>
                      ...
                      <td className="hidden lg:table-cell px-3 py-4 w-24">
                        <select 
                          className="h-10 text-sm border rounded px-2 w-full bg-white"
                          value={editInlineForm.purchasePrice}
                          onChange={e => setEditInlineForm({...editInlineForm, purchasePrice: e.target.value})}
                        >
                      ...
                      <td className="px-2 py-2 w-20">
                        <div className="flex items-center justify-center gap-1">
                          <Button variant="ghost" size="sm" className="h-7 w-7 p-0 bg-green-100" title="Kaydet" onClick={() => handleInlineUpdate(p.id)}>
                            <Check className="h-4 w-4 text-green-600" />
                          </Button>
                          <Button variant="ghost" size="sm" className="h-7 w-7 p-0 bg-gray-100" title="İptal" onClick={() => setEditingId(null)}>
                    ...
                    <tr key={p.id} className="border-b hover:bg-gray-50">
                      <td className="px-3 py-4 w-28">
                        {p.code
                          ? <span className="font-mono text-sm text-gray-900 whitespace-nowrap">{p.code}</span>
                          : <span className="text-gray-300 text-sm">—</span>}
                      </td>
                      <td className="px-3 py-4 w-28">
                        {p.imageUrl ? (
                          <img src={p.imageUrl} alt={p.name} className="h-24 w-24 object-contain rounded border bg-white" />
                        ) : (
                          <div className="h-24 w-24 bg-gray-100 rounded flex items-center justify-center text-base text-gray-400">Yok</div>
                        )}
                      </td>
                      <td className="px-3 py-4 font-medium text-sm min-w-[200px] max-w-[300px] whitespace-normal break-words leading-relaxed">
                        {p.name}
                      </td>
                      ...
                      <td className="px-3 py-4 w-28">
                        <div className="flex items-center justify-center gap-1">
                          <Button variant="ghost" size="sm" className="h-7 w-7 p-0" title="Düzenle" onClick={() => handleEdit(p)}>
                            <Pencil className="h-4 w-4 text-blue-500" />
                          </Button>
                          <Button variant="ghost" size="sm" className="h-7 w-7 p-0" title="Sil" onClick={() => handleDelete(p.id, p.name)}>
                  ...
                  {showInlineForm && (
                    <tr className="border-b bg-blue-50" onKeyDown={(e) => {
                      ...
                      <td className="hidden lg:table-cell px-3 py-4 w-24">
                        <select 
                          className="h-10 text-sm border rounded px-2 w-full bg-white"
                          value={inlineForm.purchasePrice}
                          onChange={e => setInlineForm({...inlineForm, purchasePrice: e.target.value})}
                        >
                      ...
                      <td className="px-3 py-4 w-28">
                        <div className="flex items-center justify-center gap-1">
                          <Button variant="ghost" size="sm" className="h-7 w-7 p-0 bg-green-100" title="Kaydet" onClick={handleInlineSubmit}>
                            <Check className="h-4 w-4 text-green-600" />
                          </Button>
                          <Button variant="ghost" size="sm" className="h-7 w-7 p-0 bg-gray-100" title="İptal" onClick={() => setShowInlineForm(false)}>
                            <span className="text-xs text-gray-600">✕</span>
                          </Button>
                        </div>
                      </td>
      ...
      <Dialog open={!!confirmAction} onOpenChange={() => setConfirmAction(null)}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter className="gap-2">
            ...
            <Button variant="destructive" onClick={() => confirmAction?.onConfirm()} className="flex-1">Evet, Pasife Al</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
