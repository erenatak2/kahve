      ...
      {isOpen && (
        <div className="absolute bottom-16 right-0 w-[350px] sm:w-[400px] h-[500px] bg-white rounded-2xl shadow-2xl border border-indigo-100 flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50">
            {messages.map((m, i) => {
              const orderMatch = m.content.match(/\[\[CREATE_ORDER:(.*?)\]\]/)
              const orderData = orderMatch ? JSON.parse(orderMatch[1]) : null
              const cleanContent = m.content.replace(/\[\[CREATE_ORDER:.*?\]\]/g, '')
              ...
              return (
                <div key={i} className={cn(
                  ...
                  <div className={cn(
                    "max-w-[90%] p-3 rounded-2xl text-sm shadow-sm prose prose-slate max-w-none",
                    m.role === 'user' 
                      ? "bg-indigo-600 text-white rounded-tr-none prose-invert" 
                      : "bg-white text-slate-800 border border-slate-100 rounded-tl-none"
                  )}>
                    {m.role === 'user' ? cleanContent : (
                      <ReactMarkdown 
                        components={{
                          ...
                          th: ({children}) => <th className="bg-slate-50 px-3 py-2 text-left font-bold text-slate-600">{children}</th>,
                  ...
                  {/* Aksiyon Butonu (Eğer sipariş taslağı varsa) */}
                  {m.role === 'model' && orderData && (
                    <div className="mt-2 ml-4">
                      <Button 
                        size="sm" 
                        onClick={async () => {
                          setIsCreatingOrder(`msg-${i}`)
                          try {
                            ...
                            if (response.ok) {
                              toast({ title: 'Başarılı', description: 'Sipariş taslağı başarıyla oluşturuldu.' })
                              setMessages(prev => {
                                const newMessages = [...prev]
                                newMessages[i] = { 
                                  ...newMessages[i], 
                                  content: newMessages[i].content.replace(/\[\[CREATE_ORDER:.*?\]\]/g, '') + '\n\n✅ **Sipariş başarıyla oluşturuldu!**'
                                }
                                return newMessages
                              })
                            } else {
                        ...
                        className="bg-green-600 hover:bg-green-700 text-white text-xs font-bold gap-2 rounded-xl shadow-md border-b-4 border-green-800 active:border-b-0 active:translate-y-1 transition-all"
                      >
                        <ShoppingCart className="h-3.5 w-3.5" />
                        {isCreatingOrder === `msg-${i}` ? 'İşleniyor...' : 'Siparişi Sisteme Kaydet'}
                      </Button>
                    </div>
                  )}
                </div>
